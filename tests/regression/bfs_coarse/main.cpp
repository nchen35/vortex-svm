// Pointer-linked SVM BFS — COARSE-GRAINED buffer SVM.
//
// Same pointer-linked graph and kernel as bfs_fine, but coarse-grained: the
// host transfers ownership explicitly with vx_svm_map / vx_svm_unmap. The graph
// (nodes + neighbor lists) is staged to the device once; the shared `changed`
// flag is mapped/unmapped every BFS level. This is the explicit-ownership
// counterpart used to compare against the map-free fine-grained version.
...
int main(int argc, char** argv) {
  ...
  std::cout << "SVM BFS (coarse-grained): num_nodes=" << num_nodes << "\n";
  ...
  // ── coarse-grained SVM allocations ────────────────────────────────────────
  ...
  int cflags = VX_MEM_READ_WRITE; // no VX_SVM_FINE_GRAINED

  RT_CHECK(vx_svm_alloc(device, nodes_bytes, cflags, &nodes_svm));
  RT_CHECK(vx_svm_alloc(device, nbrs_bytes,  cflags, &nbrs_svm));
  RT_CHECK(vx_svm_alloc(device, flag_bytes,  cflags, &flag_svm));
  ...
  // ── build the graph, then unmap to push it to the device (explicit) ───────
  RT_CHECK(vx_svm_map(device, nodes_svm, nodes_bytes, VX_MEM_WRITE));
  RT_CHECK(vx_svm_map(device, nbrs_svm,  nbrs_bytes,  VX_MEM_WRITE));
  ...
  int32_t level = 0;
  for (;;) {
    RT_CHECK(vx_svm_map(device, flag_svm, flag_bytes, VX_MEM_WRITE));
    *reinterpret_cast<int32_t*>(flag_svm) = 0;
    RT_CHECK(vx_svm_unmap(device, flag_svm, flag_bytes));
    ...
    RT_CHECK(vx_svm_map(device, flag_svm, flag_bytes, VX_MEM_READ));
    int32_t changed = *reinterpret_cast<int32_t*>(flag_svm);
    RT_CHECK(vx_svm_unmap(device, flag_svm, flag_bytes));
    if (!changed) break;
    ++level;
  }

  // ── map the node array back to read final costs, then verify ──────────────
  RT_CHECK(vx_svm_map(device, nodes_svm, nodes_bytes, VX_MEM_READ));
